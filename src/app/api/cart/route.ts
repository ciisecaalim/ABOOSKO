...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await getDb()
      ...
      .limit(1);
    if (existing[0]) {
      await getDb().update(cartItems).set({ qty: Math.min(9, (existing[0].qty || 1) + qty) }).where(eq(cartItems.id, existing[0].id));
    } else {
      await getDb().insert(cartItems).values({ sessionId, productId: prow[0].id, size, qty });
    }
    const lines = await linesFor(sessionId);
    return NextResponse.json({ lines });
  } catch (e) {
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (qty <= 0) {
      await getDb().delete(cartItems).where(and(eq(cartItems.id, id), eq(cartItems.sessionId, sessionId)));
    } else {
      await getDb().update(cartItems).set({ qty: Math.min(9, qty) }).where(and(eq(cartItems.id, id), eq(cartItems.sessionId, sessionId)));
    }
    const lines = await linesFor(sessionId);
    return NextResponse.json({ lines });
  } catch (e) {
