...
export async function POST(req: NextRequest) {
  try {
    ...
    let wished = true;
    if (existing[0]) {
      await db.delete(wishlistItems).where(and(eq(wishlistItems.sessionId, sessionId), eq(wishlistItems.productId, prow[0].id)));
      wished = false;
    } else {
      await db.insert(wishlistItems).values({ sessionId, productId: prow[0].id });
    }
    const items = await itemsFor(sessionId);
